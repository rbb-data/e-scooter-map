    ...
    xmlns='http://www.w3.org/2000/svg'>
      ...
      <g key={i} onClick={() => onClick(sorted_keys[i])}>
        <rect
          ...
          width={1 - 2 * margin}
					height={val * 2.5}
          fill={sorted_keys[i] === highlight ? gold : darkGrey} />
				{ i % 2 === 0 &&
					<text className={_.text} x={i + 0.5} y='3.5' textAnchor='middle' fill={darkGrey}>{ new Date(sorted_keys[i]).getDate() }</text>
				}
				{ (new Date(sorted_keys[i]).getDate() === 18) && 
					<text className={_.text} x={i + 1.86} y='4.4' textAnchor='middle' fill={darkGrey}>Dezember</text>
				}
      </g>
    )}
  </svg>
